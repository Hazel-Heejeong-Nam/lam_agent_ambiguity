...
def _model_texture_data(model):
    if hasattr(model, "tex_rgb"):
        return model.tex_rgb
    return model.tex_data


def _model_texture_nchannel(model, texture_index):
    if hasattr(model, "tex_nchannel"):
        return int(model.tex_nchannel[texture_index])
    return 3
...
class DistractingBackgroundEnv(control.Environment):
    ...
    **NOTE**: This wrapper should be applied BEFORE the pixel wrapper to make sure
    ...
    def _reset_background(self):
        ...
        sky_nchannel = _model_texture_nchannel(self._env.physics.model, SKY_TEXTURE_INDEX)
        sky_size = sky_height * sky_width * sky_nchannel
        sky_address = self._env.physics.model.tex_adr[SKY_TEXTURE_INDEX]

        texture_data = _model_texture_data(self._env.physics.model)
        sky_texture = texture_data[sky_address : sky_address + sky_size].astype(np.float32)
            ...
            if self._shuffle_buffer_size:
                ...
                if not file_names:
                    raise FileNotFoundError(
                        f"No .jpg background frames found in {video_path}. "
                        "Check that the DCS background path points to DAVIS/JPEGImages/480p, not annotations."
                    )
    ...
    def _apply(self):
        ...
        if self._background:
            ...
            _model_texture_data(self._env.physics.model)[start:end] = texture

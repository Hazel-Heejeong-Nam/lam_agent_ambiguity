...
DIFFICULTY_SCALE = dict(
    ...
    medium=0.2,
    hard=0.3,
    scale_easy_video_hard=0.1,
)
DIFFICULTY_NUM_VIDEOS = dict(
    vanilla=0,
    easy=4,
    medium=8,
    hard=None,
    scale_easy_video_hard=None,
)
...
def get_camera_kwargs(domain_name, scale, dynamic):
    assert domain_name in ["reacher", "cartpole", "finger", "cheetah", "ball_in_cup", "walker", "hopper", "humanoid"]
    assert scale >= 0.0
    assert scale <= 1.0
    return dict(
...
def get_background_kwargs(
    domain_name, num_videos, dynamic, dataset_path, dataset_videos=None, shuffle=False, video_alpha=1.0
):
    assert domain_name in ["reacher", "cartpole", "finger", "cheetah", "ball_in_cup", "walker", "hopper", "humanoid"]

...
def get_lookat_xmat_no_roll(agent_pos, camera_pos):
    ...
    pitch += np.pi / 2.0  # Camera starts out looking at [0, 0, -1.]
...
def get_lookat_xmat(agent_pos, camera_pos):
    ...
    delta_vec = agent_pos - camera_pos
    delta_vec /= np.sqrt(np.sum(delta_vec**2.0))
    y_vec = np.array([0.0, 0.0, -1.0])  # This is where the cam starts from.
    a = np.cross(y_vec, delta_vec)
    sin_theta = np.sqrt(np.sum(a**2.0))
    cos_theta = np.dot(delta_vec, y_vec)
    a /= np.sqrt(np.sum(a**2.0)) + 0.0001
    return _mat_from_theta(cos_theta, sin_theta, a)
...
def clip_cam_position(position, min_radius, max_radius, min_h_angle, max_h_angle, min_v_angle, max_v_angle):
...
def get_lookat_point(physics, camera_id):
    ...
    """
    dist_to_agent = physics.named.data.cam_xpos[camera_id] - physics.named.data.subtree_com[1]
...
class DistractingCameraEnv(control.Environment):
    ...
    **NOTE**: This wrapper should be applied BEFORE the pixel wrapper to make sure
    ...
    def setup_camera(self):
        ...
        self._cam_initial_lookat_point = get_lookat_point(self._env.physics, self._camera_id)

        start_pos = copy.deepcopy(self._env.physics.named.data.cam_xpos[self._camera_id])
    ...
    def _apply(self):
        ...
        if self._camera_type == "fixed":
            ...
            lookat_mat = lookat_method(get_lookat_point(self._env.physics, self._camera_id), cart_cam_pos)

        lookat_mat = np.reshape(lookat_mat, (3, 3))
        roll_mat = rotvec2mat(self._roll, np.array([0.0, 0.0, 1.0]))
